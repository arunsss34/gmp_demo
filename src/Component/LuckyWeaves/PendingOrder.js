...
const PendingOrder = () => {
    ...
    useEffect(() => {
        if (searchWord) {
          const filtered = filteredItems.filter(item=> 
            item['Po No'].toLowerCase().includes(searchWord.toLowerCase()) ||
    ...
    const getLud = async () => {
        try {
            const response = await getFromAPI(`customer_pending_order_dates_lw`);
    ...
    async function fetchFrameItems() {
        ...
        try {
            ...
            const response = await getFromAPI(`/get_customer_po_details_lw?data=${JSON.stringify(data)}`);
    ...
    const PdfDownload = async () => {
        ...
        try {
            ...
            const response = await getFromAPI(`/get_customer_po_pdf_lw?data=${JSON.stringify(data)}`);
