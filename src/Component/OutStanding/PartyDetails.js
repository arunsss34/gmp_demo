...
export default function PartyDetails({ onNavigate,searchWord,setSearchWord }) {
  ...
  useEffect(() => {
    ...
    async function fetchData(day) {
        setLoading(true);
        try {
            const data = { menu_pk, agent_id:company_id, customer_id: party_id,day,company };
            const response = await getFromAPI("/get_party_details_by_agent?data=" + JSON.stringify(data));
    ...
    const handleDropdownChange = (e) => {
      ...
      async function PdfDownload() {
        ...
        setPdfLoading(true)
        try {
            const data = { menu_pk,agent_id:company_id,customer_id: party_id,company };
            const response1 = await getFromAPI("/get_outstanding_party_details_overview_pdf?data=" + JSON.stringify(data));
