...
export default function PartyInfo({ onNavigate,searchWord,setSearchWord  }) {
  ...
  async function fetchData(day) {
      ...
      try {
          ...
          const response1 = await getFromAPI("/get_party_info?data=" + JSON.stringify(data));
          setAgentName(response1.agent_name)

          response1.party_info.forEach((party) => {
  ...
  async function PdfDownload(download_pk) {
    setLoading(true);
    try {
        const data = { menu_pk,download_pk,agent_id };
        const response1 = await getFromAPI("/get_outstanding_party_overview_pdf?data=" + JSON.stringify(data));
          ...
          {loading ? (
            ...
            <div>
              ...
              <div style={{ marginBottom: '8px',fontSize:12}}>
                  <span style={{fontWeight: 'bold', opacity:.8 }}>Agent: </span>
                            ...
                            <Grid container spacing={2}>
                              {section.details.map((item, index) => (
                                <Grid item xs={12} md={4} key={index}>
                                  <Card onClick={() => handleNavigate(item.CustomerID,item.Company) } style={{ background: currentTheme.background }}>
                                    <CardContent>
                                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 12 }}>
                                        <span style={{ fontWeight: 'bold', color: currentTheme.text }}>{item.CustomerName}</span>
