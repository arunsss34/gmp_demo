...
export default function AgentInfo({ onNavigate,searchWord,setSearchWord }) {
  ...
  async function fetchData(day) {
      ...
      try {
          ...
          const response1 = await getFromAPI("/party_otd_get_agent_details?data=" + JSON.stringify(data));
  ...
  const handleNavigate = (agentID) => {
    ...
    onNavigate('Agent Info', 'Agent Info');
  };
  ...
  async function PdfDownload(download_pk) {
    ...
    try {
        ...
        const response1 = await getFromAPI("/party_get_outstanding_overview_pdf?data=" + JSON.stringify(data));
