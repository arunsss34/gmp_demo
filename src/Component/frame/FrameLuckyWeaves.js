...
const StockReport = () => {
    ...
    const getLud = async () => {
        try {
            const response = await getFromAPI(`get_frame_last_update_dt_lw`);
    ...
    async function fetchFrameItems() {
        ...
        try {
            ...
            const data = { menu_pk: menu_pk, type: 3, date: formattedDate }; 
            const response = await getFromAPI(`/get_stock_details_lw?data=${JSON.stringify(data)}`);
    ...
    async function PdfDownload() {
        ...
        try {
            ...
            const formattedDate = `${date2.getFullYear()}-${('0' + (date2.getMonth() + 1)).slice(-2)}-${('0' + date2.getDate()).slice(-2)}`;
            const data = { menu_pk: menu_pk, type: 3, date: formattedDate }; 
            const response = await getFromAPI(`/get_stock_pdf_lw?data=${JSON.stringify(data)}`);
