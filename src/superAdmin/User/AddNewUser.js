...
export default function AddNewUser({setDialogVisible,handleRefresh}) {
...
function sleep(ms) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await postToAPI("/add_user", data);
      if (result.rval> 0) {
        ...
        <>
          <form onSubmit={handleSubmit}>
            ...
            <Grid item xs={12} md={4}>
                <Button style={{background:'#48a1eb;', color: 'white'}} label="Save" icon="pi pi-check" size="small" type="submit"/>
                &nbsp;&nbsp;
            </Grid>
          </form><br/>
